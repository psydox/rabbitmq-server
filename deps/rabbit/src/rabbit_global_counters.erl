...
init(#{protocol := Protocol, queue_type := QueueType}) ->
    _ = seshat_counters:new_group(protocol_queue_type),
    Counters = seshat_counters:new(protocol_queue_type, {Protocol, QueueType}, ?PROTOCOL_QUEUE_TYPE_COUNTERS),
    persistent_term:put({?MODULE, Protocol, QueueType}, Counters),
    ok;
init(#{protocol := Protocol}) ->
    _ = seshat_counters:new_group(protocol),
    Counters = seshat_counters:new(protocol, Protocol, ?PROTOCOL_COUNTERS),
...
overview() ->
    maps:merge(
      overview(protocol),
      overview(protocol_queue_type)
    ).

overview(Group) ->
    seshat_counters:overview(Group).

prometheus_format(Group) ->
    seshat_counters:prometheus_format(Group).
